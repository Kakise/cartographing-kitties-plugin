...
Built on :mod:`cartograph.planning.state`; the parser is the single source of
truth for plan format and mutation semantics. Mutating subcommands read the
file's sha256 before parsing and pass it to
:func:`cartograph.planning.state.write_plan_atomic` so concurrent edits abort
with a clear message instead of clobbering changes.
"""
...
def _repo_root() -> Path:
    """Locate the repository root by walking up for a ``pyproject.toml``.

    The CLI now ships inside the package, so it can no longer assume a fixed
    ``scripts/`` parent. Prefer the current working directory (and its parents)
    so the tool works from anywhere inside a checkout; fall back to cwd.
    """
...
def cmd_set_unit(args: argparse.Namespace) -> int:
    ...
    if errors:
        ...
        return 1
    try:
        write_plan_atomic(path, plan, expected_hash=expected_hash)
...
# Backwards-compatible private alias (the old module exposed ``_build_parser``).
