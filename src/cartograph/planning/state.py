...
The module is mostly side-effect free apart from file reads in
:func:`parse_plan` and the atomic write in :func:`write_plan_atomic`. The CLI
lives in :mod:`cartograph.planning.cli`.
"""
...
def set_unit_state(
    ...
    *,
    ...
    # ``commit`` is an alias for ``implemented_in`` matching the CLI/MCP surface
    # (``--commit`` flag / ``commit`` tool arg). ``implemented_in`` wins if both
    # are supplied so existing callers keep their behavior.
    if implemented_in is None:
        implemented_in = commit
...
def write_plan_atomic(path: Path, plan: Plan, expected_hash: str | None = None) -> None:
    """Serialize *plan* and replace *path* atomically.

    When *expected_hash* is provided and the file exists, the on-disk content's
    sha256 is checked against it first; a mismatch raises
    :class:`PlanConflictError` (optimistic concurrency guard). The new content
    is written to a temp file in the same directory, then ``os.replace`` swaps
