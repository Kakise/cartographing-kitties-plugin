...
name: kitty-review
description: |
  Structured code review using Cartographing Kittens-powered reviewer agent swarms. Use when the user says "review this", "check my code", "code review", or before creating a PR. Uses inline-first structural review with optional reviewer delegation, merges findings, and optionally applies safe fixes. Supports modes: interactive (default), autofix (mode:autofix), report-only (mode:report-only). Requires the Cartographing Kittens MCP server (`uvx cartographing-kittens`).
when_to_use: |
  Triggers: "review this", "check my changes", "audit the diff", "find bugs", "pre-PR review", "/kitty:review", "kitty review mode:autofix". Runs after `kitty:work` and before merge.
...
- mcp__plugin_kitty_kitty__batch_query_nodes
...
- mcp__plugin_kitty_kitty__search
...
### Stage 4: Index & dispatch the review fan-out

1. Call `index_codebase(full=false)` so the graph reflects current changes (a run boundary).
2. Write the curated Stage 3 subgraph context as the Context Bundle to
   `.pawprints/runs/<run-id>/bundle.md` per
   [`kitty/references/bundle-format.md`](kitty/references/bundle-format.md), then
   dispatch the review fan-out via the orchestration script
   [`kitty/references/workflows/review.orch.js`](kitty/references/workflows/review.orch.js).
   The conductor computes each dimension's `(model, effort)` tier from
   [`kitty/references/dispatch-policy.md`](kitty/references/dispatch-policy.md) and
   passes the resolved tiers in `args`; the MCP-free reviewer agents read one bundle
   section each and never call MCP.

The script runs `pipeline(dimensions, review → adversarial-verify)`:
...
**Conditional:**
- **`expert-kitten-impact`** — diff touches 3+ files or modifies public symbols
- **`expert-kitten-structure`** — new files created or module boundaries change
- **`expert-kitten-context`** — Context Bundle exceeds ~10k tokens (audits over-fetching)

**Two-gate review + adversarial verify.** Every finding is judged on both the
spec-compliance gate (does the change do what it claims?) and the code-quality gate. Each
P0/P1 finding is then adversarially re-verified at the hard tier (`opus`, `xhigh`) before
it is kept — a finding that fails verification is dropped. A reviewer that throws / returns
empty / returns a schema-invalid result is recorded `failed` in the per-item ledger and
re-dispatched once, never silently dropped (spec §6).

On the Task fallback path the orchestrator runs the same dimensions and validates each
return against the schema; model tiering is advisory-only there (#43869), effort travels
as a prompt hint.
...
- **Orchestration script:** `kitty/references/workflows/review.orch.js`
