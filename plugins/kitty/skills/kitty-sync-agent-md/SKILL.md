...
description: |
  Audit CLAUDE.md and AGENTS.md (dual-runtime project memory files) for section-structure drift. Use when the user says "sync AGENTS.md", "check CLAUDE/AGENTS drift", "are CLAUDE.md and AGENTS.md aligned", "/kitty:sync-agent-md". Works on any project that uses both files for parallel Claude Code and Codex/OpenAI-flavored runtimes.
when_to_use: |
  Triggers: "sync agent md", "CLAUDE.md AGENTS.md drift", "agents md is out of date", "check both memory files", "/kitty:sync-agent-md".
argument-hint: '[--fix]'
allowed-tools:
- Bash
- Read
