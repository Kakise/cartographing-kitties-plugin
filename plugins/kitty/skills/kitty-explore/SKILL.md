...
- mcp__plugin_kitty_kitty__get_context_summary
...
metadata:
  short-description: Browse codebase structure — definitions, imports, calls, inheritance — through the
    AST graph.
requires:
  mcp_servers:
  - kitty
...
The orchestrator builds all context directly — exploration is orchestrator-inline and needs no
agent dispatch. Agents CANNOT call MCP tools, so the orchestrator IS the explorer. For an
unusually large or dense area the orchestrator MAY dispatch a single `librarian-kitten`
(architecture lens) over a Context Bundle to synthesize the summary, but this is an optional
optimization — there is no `explore.orch.js` and no required fan-out.
