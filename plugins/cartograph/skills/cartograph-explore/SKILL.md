---
name: cartograph:explore
description: >
  Structural codebase exploration using Cartograph's graph. Use when the user asks
...
Explore codebase structure through the Cartograph graph — definitions, imports,
...
- For semantic queries ("find auth code"), check `annotation_status` first — if many
  nodes are pending, run `cartograph:annotate` before searching
