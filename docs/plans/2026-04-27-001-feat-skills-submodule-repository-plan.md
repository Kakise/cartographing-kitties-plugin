---
title: Skills Vendored skill source Repository (JetBrains-compatible)
type: feat
status: complete
date: 2026-04-27
origin: docs/brainstorms/2026-04-27-001-skills-vendored skill source-repository-requirements.md
implemented_in: d7ec32f
---

# Skills Vendored skill source Repository — Implementation Plan
...
repo as a vendored generated skill tree mounted at `plugins/kitty/skills/`. The MCP server, plugin
...
flat top-level vendored skill source layout and a two-layer host-capability signal (`requires`
frontmatter + explicit description sentence). This plan turns that into concrete units.

## Requirements Trace

The brainstorm document at `docs/brainstorms/2026-04-27-001-skills-vendored skill source-repository-requirements.md`
...
| R5 (vendored generated skill tree integration; src/cartograph/ stays) | U3 |
...
**In scope**
- New vendored skill source repo bootstrap (skills + agents + manifest + LICENSE + README + CI)
- Frontmatter migration for all 9 skills (`requires`, `metadata.source`, hard-requirement sentence)
- Wiring this repo to consume the vendored skill source at `plugins/kitty/skills/`
...
| `.github/workflows/ci.yml:78-93` | `find plugins/kitty/skills -mindepth 1 -maxdepth 1 -type d` (count ≥ 9) and `find plugins/kitty/agents -name '*.md'` | **High** — vendored skill source must be checked out before this runs |
...
| `.codex-plugin/plugin.json` (root) | `"skills": "./plugins/kitty/skills"`, `"mcpServers": "./.mcp.json"` | Medium — skills path unchanged after vendored skill source mount; mcpServers path needs verification |
...
| `AGENTS.md:30`, `legacy runtime docs:17` | descriptions of `plugins/kitty/` | Low — wording updates |
| `CLAUDE.md:92` | "Codex preserves the same subagents through `plugins/kitty/agents/manifest.json`" | Low — wording update |
| `docs/architecture/repo-boundaries.md` | enumerates `plugins/kitty/` integration ownership | Medium — must articulate the vendored skill source boundary |
...
1. **Single vendored skill source mount at `plugins/kitty/skills/`.** The existing path is reused; vendored skill source contents replace the directory's contents directly because both layouts are flat and use `kitty-*` naming.

2. **Agents discovery — Option A primary, Option B fallback.**
   - **Option A (preferred):** add `"agents": "./skills/agents"` to `plugins/kitty/.claude-plugin/plugin.json`. Agents are discovered from inside the vendored skill source. Single source of truth. Requires: Claude Code's plugin schema supports a configurable `agents` path (it already supports `"commands": "./commands"`, so symmetric support is plausible but unverified). U3 includes an empirical check; if the field is ignored or fails to load, fall back to Option B without re-litigating other units.
...
3. **`requires` frontmatter schema.** Defined and validated in the vendored skill source:
   ```yaml
   requires:
     mcp_servers:
       - kitty
...
4. **`metadata.source` URL points at the vendored skill source.** When the vendored skill source is later mirrored into JetBrains/skills, mirror entries will reference back to the vendored skill source via the same field — matching JetBrains's curation convention.

5. **Single root LICENSE in vendored skill source.** MIT, matching the existing plugin license. Per-skill `LICENSE.txt` files are deferred unless a future skill needs a different license.

6. **Vendored skill source name = `vendored-skill-sources`.** Aligns with the existing `cartographing-kitties-plugin` brand referenced in `homepage`/`repository` URLs across plugin manifests.
...
- Automated semver release tags for the vendored skill source catalog.

## Implementation Units

### U1. Bootstrap the new vendored skill source repository

**Goal.** Create `vendored-skill-sources` as a standalone repo with the flat
...
- `agents/manifest.json` — copied verbatim from `plugins/kitty/agents/manifest.json` (paths and runtime annotations remain valid because the vendored skill source preserves the relative layout `agents/<name>.md`).
...
**Approach.**
1. Create the new repo locally (`git init`, set remote to `https://github.com/vendored skill sources`).
...
4. For every `SKILL.md`, update the frontmatter:
   ...
   - Add `metadata.source: https://github.com/vendored skill sources/blob/main/<skill-dir>/SKILL.md`.
...
- *Reference integrity:* relative links inside `kitty/SKILL.md` to `references/*.md` resolve from the vendored skill source root (not the old `plugins/kitty/skills/kitty/` path).
...
### U2. Add JetBrains-compatible CI and frontmatter validator to the vendored skill source

**Goal.** Mirror JetBrains/skills's CI posture so the vendored skill source is catalog-eligible from
...
**Files to create (in the vendored skill source):**
...
- `.scripts/validate-frontmatter.py` — Python script (stdlib + PyYAML) that walks every `*/SKILL.md`, parses frontmatter, asserts:
  ...
  - `metadata.source` is a string starting with `https://github.com/vendored skill sources/`
...
- This product repo's CI uses `uv` and `pytest`; the vendored skill source uses raw Python because it has no Python project — keeping dependencies minimal (PyYAML only).
...
### U3. Mount the vendored skill source into this repo and resolve agents discovery

**Goal.** Replace this repo's in-tree `plugins/kitty/skills/` and `plugins/kitty/agents/`
with content sourced from the vendored skill source, while keeping `plugin.json`, `commands/`, the
MCP server, and runtime manifests in place.

**Requirements advanced.** R4, R5, R6, R7, R11.

**Dependencies.** U1 (vendored skill source repo must exist with content).

**Files to create or modify in this product repo:**
- `.gitmodules` — new entry registering the vendored skill source:
  ```
  [vendored skill source "plugins/kitty/skills"]
      path = plugins/kitty/skills
      url = https://github.com/vendored skill sources.git
  ```
- `plugins/kitty/skills/` — removed from this repo's tracking (`git rm -r`) and added back as a vendored skill source pointing at the new repo.
...
2. `git rm -r plugins/kitty/skills` and commit; the new vendored skill source will replace the path.
3. `vendored generated skill tree add https://github.com/vendored skill sources.git plugins/kitty/skills`.
...
7. Reconcile root `.codex-plugin/plugin.json`'s `"mcpServers": "./.mcp.json"` with the deleted-from-working-tree `.mcp.json` (Q2): either restore the root file or repoint the manifest. Out of strict scope for the vendored skill source extraction but blocks CI green; resolve here so Unit U4's tests pass.

**Patterns to follow.**
- Existing `plugins/kitty/.claude-plugin/plugin.json` already declares `"commands": "./commands"` — use the same syntax shape for `"agents"`.
- Use `vendored generated skill tree update --init --recursive` as the documented bootstrap command (mentioned in U5 docs).

**Test scenarios.**
- *Happy path A:* fresh clone + `vendored generated skill tree update --init` populates `plugins/kitty/skills/` and (via the vendored skill source's `agents/` subdir) makes framework agents discoverable to Claude Code at startup.
...
- *Edge case — vendored skill source not initialized:* `plugins/kitty/skills/` is empty; CI fails fast with a clear error pointing at the missing `vendored generated skill tree update --init` step.
- *Edge case — vendored skill source pin drift:* updating the vendored skill source SHA produces a diff in this repo that explicitly references the SHA bump, never the file contents.
- *Negative — Option A misconfigured:* if `"agents": "./skills/agents"` is set but the path is empty (vendored skill source uninit), Claude Code surfaces no framework agents → bootstrap docs (U5) cover this.

**Verification.**
- `vendored generated skill tree status` lists `plugins/kitty/skills` with the expected URL.
...
the vendored skill source-mounted layout and still validate the same invariants (skill count, agent
count, manifest integrity).

**Requirements advanced.** R5, R7, R9.

**Dependencies.** U3 (vendored skill source mounted, agents discovery resolved).
...
- `tests/test_plugin_packaging.py`
  ...
  - Add a new test `test_skills_vendored skill source_initialized` — asserts `plugins/kitty/skills/.git` exists (a file in vendored skill source mode) or the directory is non-empty with at least one `SKILL.md`.
  ...
  - (Option B only) Add `test_agents_dir_in_sync_with_vendored skill source` — runs `scripts/sync-skills-agents.py --check` and expects exit 0.
- `.github/workflows/ci.yml`
  - Add `vendored skill sources: recursive` to every `actions/checkout@v6` step that needs the skills/agents paths.
...
- `.github/workflows/publish.yml` — add `vendored skill sources: recursive` to checkout if any publish step touches the skills/agents paths (currently it only validates `plugin.json`, but verify and adjust).
...
4. Verify both happy-path checkout (with vendored skill source) and a deliberate failure mode (CI without vendored skill source init) produce expected outcomes.
...
- *Negative — missing vendored skill source:* a deliberately broken checkout (no `--init`) makes `test_skills_vendored skill source_initialized` fail with a clear error.
- *Negative — drifting agents (Option B):* corrupt `plugins/kitty/agents/cartographing-kitten.md`; `test_agents_dir_in_sync_with_vendored skill source` fails and points at the corrupted file.
- *Integration:* full CI run on a PR succeeds; same PR with `vendored skill sources: recursive` removed from `actions/checkout` fails fast in `validate-plugin`.
...
### U5. Update documentation, AGENTS/CLAUDE/legacy runtime docs, and architecture contracts
...
**Files to modify:**
- `README.md` — add a "Skills Vendored skill source" section after the existing plugin layout description (current lines 71-127). Cover: what the vendored skill source is, why it exists, the JetBrains-AI-Assistant compatibility, and the bootstrap commands (`vendored generated skill tree update --init --recursive`, `uv sync`). Update lines that describe `plugins/kitty/skills` so they note the vendored skill source.
- `CLAUDE.md` — update the "Plugin Structure (Marketplace Layout)" tree to show `plugins/kitty/skills/` as a vendored skill source mount and note the configurable `agents` path (Option A) or the sync script (Option B). Add a one-line link to the canonical skills repo.
- `AGENTS.md` — at line 30, append a sentence: skills + framework agents now live in the `vendored-skill-sources` vendored skill source; this repository is the integration host.
- `legacy runtime docs` — same update at line 17.
- `docs/architecture/repo-boundaries.md` — add a "Skills Catalog" sub-section under Integration: documents the vendored skill source URL, the mount point, and the rule that this product repo never edits skill content directly (PRs against the vendored skill source instead).
- `docs/architecture/codex-workflow-contract.md` — update the Runtime Model table and the Canonical Agent Declaration section so the source-of-truth path is `plugins/kitty/skills/agents/manifest.json` (Option A) or remains `plugins/kitty/agents/manifest.json` with a note that it is generated from the vendored skill source (Option B).
...
**Approach.**
1. Draft the README "Skills Vendored skill source" section first; it anchors the rest.
...
- *Cross-reference integrity:* every link to the new vendored skill source resolves (manual click-through during review).
- *Onboarding test:* fresh clone followed verbatim by README's bootstrap section yields a working setup (manual rehearsal).

**Verification.**
- `uv run codespell src docs README.md CLAUDE.md AGENTS.md legacy runtime docs` is clean.
...
- **CI:** Both workflows gain `vendored skill sources: recursive`; `validate-plugin` job is unchanged in structure but reads from a vendored skill source-mounted path.
- **Documentation:** 6 user-facing docs touched (README, CLAUDE.md, AGENTS.md, legacy runtime docs, two architecture docs).
- **Runtime behavior:** Unchanged for end users beyond the `vendored generated skill tree update --init` step on first clone. The `kitty:` skills resolve at the same paths; the MCP server installs the same way.
- **External surface:** A new public repo at `https://github.com/vendored skill sources`.
...
| Vendored skill source pin drift: contributors update skills/agents in this repo by accident | U5 architecture contract documents the rule; pre-commit hook in U4 (Option B) detects unsynced edits; Option A makes it impossible because the path is read-only relative to this repo. |
...
| First-time clone confusion (skills appear empty) | U5 README explicitly documents `vendored generated skill tree update --init --recursive`. |
...
- Origin requirements: `docs/brainstorms/2026-04-27-001-skills-vendored skill source-repository-requirements.md`
- Architecture contracts: `docs/architecture/repo-boundaries.md`, `docs/architecture/codex-workflow-contract.md`
- Existing manifests: `plugins/kitty/.claude-plugin/plugin.json`, `plugins/kitty/.codex-plugin/plugin.json`, `plugins/kitty/legacy-runtime-extension.json`, `plugins/kitty/.mcp.json`, `.codex-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `legacy-runtime.json`
