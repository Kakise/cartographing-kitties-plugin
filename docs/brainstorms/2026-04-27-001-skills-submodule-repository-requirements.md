# Skills Vendored skill source Repository — Requirements

## Problem Frame

Cartographing Kittens currently keeps its reusable agent workflow assets inside this repository under `plugins/kitty/`. That works for the bundled Codex, Claude Code, legacy runtime, and legacy runtime plugin layouts, but it makes the skill layer harder to publish independently and harder to reuse from tools that expect a skills catalog repository.

Create a new standalone skills repository that can be added back to this repository as a vendored generated skill tree. The new repository should be the source of truth for Cartographing Kittens skills and should also be consumable as a JetBrains AI Assistant skills repository, following the public repository shape used by `JetBrains/skills`.
...
- `plugins/kitty/legacy-runtime-extension.json` and `plugins/kitty/.mcp.json` preserve legacy runtime and MCP wiring.
...
- R5. Convert this repository to consume the new skills repository as a vendored generated skill tree, without making `src/cartograph/` depend on the vendored skill source at runtime.
- R6. Keep runtime-specific packaging glue separate from canonical skill content. Codex, Claude Code, legacy runtime, legacy runtime, and MCP manifests may reference the vendored skill source, but runtime-specific manifests should not be duplicated into every skill unless required by that runtime.
...
- R8. Update documentation so users know which repository is authoritative for skills, how to update the vendored skill source, and how to install the skills in Codex and JetBrains AI Assistant.
...
- R11. The vendored skill source ships **skills + framework agents + `agents/manifest.json`** together. The MCP server (`src/cartograph/`), Claude Code `plugin.json`, Codex plugin manifests, and runtime-specific commands (`plugins/kitty/commands/`) remain in this product repository. Skills and agents are versioned as one unit because the workflow contract (`docs/architecture/codex-workflow-contract.md`) ties them together.
- R12. Vendored skill source layout is **flat at the root**: each skill lives at `<vendored skill source>/<kitty-*>/SKILL.md`, matching the JetBrains/skills convention. Framework agents live at `<vendored skill source>/agents/` as a non-skill top-level directory (alongside `.github/`, `.scripts/`, `doc/`, etc.).
...
- R14. Each `SKILL.md` carries a `metadata.source` URL pointing at the canonical skill path inside the new vendored skill source (the JetBrains attribution convention). When/if these skills are mirrored into `JetBrains/skills`, that catalog's entries point back to the vendored skill source via the same field.
- R15. The skills vendored skill source SHOULD adopt JetBrains-style CI (Cisco `skill-scanner` via GitHub Actions producing SARIF) so it stays catalog-eligible from day one. A failing scan on changed-skill error-level findings blocks the vendored skill source's CI, not this repo's CI.
- R16. The vendored skill source must be installable and discoverable **without** the Cartographing Kittens product checkout: a clone of the vendored skill source alone is a valid JetBrains AI Assistant skills repository.

## Success Criteria

- The new repository can be cloned independently and exposes all existing Cartographing Kittens skills.
- This repository can point at that repository as a vendored generated skill tree and still install/load the Codex plugin from the expected plugin manifest path.
...
- Existing skill references such as `kitty:plan`, `kitty:review`, and `kitty:lfg` continue to resolve in Codex after the vendored skill source transition.
- A validation command fails on broken skill paths or missing referenced files.
- After vendored skill source init, `plugins/kitty/skills/<name>/SKILL.md` and `plugins/kitty/agents/<name>.md` both resolve to content sourced from the vendored skill source with no manual copy step required by an end user (one `vendored generated skill tree update --init --recursive` plus `uv sync` is enough).
...
- In scope: repository layout, vendored skill source integration, skill file migration, manifest path updates, docs, and validation.
...
- **Vendored skill source contents (resolved 2026-04-27).** Ship skills + framework agents + `agents/manifest.json` together. The MCP server, Claude Code `plugin.json`, and `commands/` stay in this repo. Rationale: skills reference framework agents by name and the workflow contract pairs them; commands are a Claude-Code-specific packaging primitive that travels with `plugin.json`.
- **Vendored skill source layout (resolved 2026-04-27).** Flat top-level — `<vendored skill source>/kitty-explore/`, `<vendored skill source>/kitty-annotate/`, etc. — plus `<vendored skill source>/agents/` as a non-skill top-level dir. This is byte-identical to the JetBrains/skills convention; existing directory names already match.
...
- **Vendored skill source mount path in this repo (recommended).** Mount the new vendored skill source at `plugins/kitty/skills/`. Vendored skill source contents replace the current contents of that directory verbatim (since both already use `kitty-*` flat layout). The vendored skill source's `agents/` subdirectory is published to `plugins/kitty/agents/` via one of two mechanisms decided in plan phase:
  - Option A (preferred if Claude Code's `plugin.json` allows it): set `"agents": "./skills/agents"` in `plugins/kitty/.claude-plugin/plugin.json` so agents are discovered from the vendored skill source path directly. Single source of truth, zero sync step.
...
- **Runtime manifests are adapters.** The skill content is canonical; `.codex-plugin`, `.claude-plugin`, legacy runtime, legacy runtime, and marketplace files should only point at or package that content.
- **Vendored skill source integration is a consumer boundary.** This repository pins a known-good skill version while the standalone skills repository releases independently.

## Open Questions

### Resolve Before Planning

- What should the new repository be named and hosted as? Candidate: `cartographing-kittens-skills` or `vendored-skill-sources`. (Brand alignment with the existing `cartographing-kitties-plugin` homepage URL in `plugin.json` would suggest `vendored-skill-sources`.)
...
- What exact JSONSchema should the `requires` frontmatter field use, and where is it validated? Candidates: a script in the vendored skill source's `.scripts/` that runs alongside the Cisco `skill-scanner` step, plus a parallel validator in this repo's existing test suite.
- Does the vendored skill source carry its own `LICENSE` (MIT, matching `plugin.json`) at the root, per-skill `LICENSE.txt` mirrors, or both?
